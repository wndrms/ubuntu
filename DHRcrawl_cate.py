...
def crawlDHR_cate(driver, cate_no, page):
    p = 1
    product_list = ''
    while p <= int(page) :
        URL = "http://danharoo.com/product/list.html?cate_no="+cate_no+"page="+str(p)
        driver.get(URL)
...
if __name__ == '__main__':
    ...
    try:
        login(driver)
        product_list = crawlDHR_cate(driver, cate_no, page)
        product_no = product_list.split(',')
        for x in product_no:
